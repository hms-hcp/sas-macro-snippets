...
Keyword parameters:
  ...
  stop    =no by default. Use stop=yes if you want the macro to stop
           ...
           and to review the matrice prepared for the proc optnet.
  ...
  caliper=0.6 -default value of caliper.
          ...
          networking algorith implemented by SAS PROC OPTNET.
...
%macro propmatchoptimal(datain,treatmn,id,blocks,predprb,dtmodel,dataout,vlist,
    listnum, numberofcontrols=1,caliper=0.6,highvalue=10000,stop=no,obs=0,noint=no,libout=,libin=);
...
/*we replaced proc assign with PROC OPTNET
...
*/
      ...
      select b.id1C as id1C, b.&blocks, 
             a.id1T as id1T, a.cost as psdistance
...
%if %upcase(&stop)=NO %then %do;/*put  all the results together if we run proc assign*/
