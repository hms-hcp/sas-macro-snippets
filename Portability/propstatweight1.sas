...
| matched pairs of records between treatment and control groups. Should be | | used for the data created using propensity score matching :              |
...
label=no (default) if label=yes the variable labels are displayed instead of
...
Example of macro call:
  ...
  %propstat(mydata,cath,hicbic,weight,&list,format=$varname.,lib=out.,save=yes)
*/
*--------------------------------------------------------------------------*;
...
*--------------------------------------------------------------------------*;
%macro propstatweight1(datain,treatmn,id,weight,varlist,label=no,lib=,save=no);
...
ods trace on;
ods output Statistics=_stats1;
proc surveymeans data=first;
var &varlist;
weight &weight;
run;
ods output Statistics=_stats2;
proc surveymeans data=second;
var &varlist;
weight &weight;
run;
ods trace off;
/*
proc print data=_stats1;
run;
*/
data _stats1;
set _stats1(keep=mean VarName) end=last;
keep %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
retain  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
array vars{*}  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
%do i=1 %to &mv_num;
newvar="&&mvar&i";
if trim(VarName)=trim(newvar) then do;
       vars{&i}=mean;
       goto end;
end;
%end;
end: if last then output;
run;
/*
proc print;
run;
*/
data _stats2;
set _stats2(keep=Mean VarName) end=last;
keep %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
retain  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
array vars{*}  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
%do i=1 %to &mv_num;
...
data firstm;
set firstm;
length %if label=no %then %str(VarName $ &varlen;);
          %else %str(VarName $ &labellen;);
...
if _stat_="STD" then do;
    ...
    %do i=1 %to &mv_num;
        grp1mean=fm{&i}*adj{&i};
        grp1std=&&mvar&i;
        %if label=no %then %str(VarName="&&mvar&i";);
          %else %str(VarName="&&mlabel&i";);
        output;
    %end;
end;

keep VarName grp1mean grp1std;
run;
...
keep VarName grp2mean grp2std;
run;

*proc print;

data firstm;
if _N_=1 then set _stats1;
set firstm;
array vars{*}  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
if _N_>1 then do;
%do i=1 %to  &mv_num;
if trim(VarName)="%trim(&&mvar&i)" then do;
       if grp1mean>1 and vars{&i}<1 then vars{&i}=vars{&i}*100;
       grp1mean=vars{&i};
/*put VarName= vars{&i}= grp1mean= ; */
end;
%end;
end;
keep VarName grp1mean grp1std;
...
data secondm;
if _N_=1 then set _stats2;
set secondm;
array vars{*}  %do i=1 %to &mv_num; %str(&&mvar&i ) %end;;
if _N_>1 then do;
%do i=1 %to &mv_num;
if trim(VarName)="%trim(&&mvar&i)" then do;
       if grp2mean>1 and vars{&i}<1 then vars{&i}=vars{&i}*100;
       grp2mean=vars{&i};
/*put VarName= vars{&i}= grp2mean= ; */
end;
%end;
end;
keep VarName grp2mean grp2std;
...
keep VarName grp1mean grp2mean stddif;
label
