...
=*/
%Global dummyvar;
*options mlogic macrogen mprint;
...
proc summary data = &out nway ;
	...
	*-- Eliminate the base category;
	data _cats_;
		...
		%else %str(if upcase(&vari) = &baseval then delete;);
	run;
	%end;
...
%if "&name" = "VAL" %then %do ;
	...
	%let vl&j =; 
	%do k=1 %to &num;
		%if &sysver < 7 & %length(&pre&&c&k) > 8 %then %do;
...
%else %do ;
	...
	%let vl&j =; 
	%do k=1 %to &num; 
		%if &sysver < 7 & %length(&pre.&k) > 8 %then %do;
			%put ERROR: Cannot generate names longer than 8 characters;
			%let abort=1;
			%goto endloop;
			%end;
