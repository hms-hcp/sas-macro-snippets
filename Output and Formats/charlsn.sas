...
%put Original program written by Sally Sharp, modified by Leonie Stranc and Anna Nelson;
...
*/
     ...
     ************ COPD *********************** WEIGHT = 1 **************;
      ...
      IF PULMON1=0 THEN DO;
         IF  '491  ' <=DX<= '49499'|'496  '= DX THEN DO;
...
/* Flag 54 removed , added in with flag 56
      ...
      IF (DX = '2730 ' | DX = '2733 ' | '203  ' <=DX<= '2038 ')
        & PARPRTCA=0 THEN DO;
         ...
         END;
*/
      IF ('196  ' <=DX<= '19999') & SECONDCA=0 THEN DO;
              %FLAGSET(SECONDCA,FLAGS{NFLAGS+1},NFLAGS,55);
         END;
      IF ('200  ' <=DX<= '20399'| DX = '2730 ' |
...
/* NOTE: as PARPRTCA has been commented out it cannot=1-it is now
      ...
      IF (('204  ' <=DX<= '20899') | PARPRTCA=1) & LEUKEMIA=0
         THEN DO;
...
/*Used in Deyo comparison (added 15.12.92)*/
...
/* weight =1 */
               ...
               ((CVP36526 | CVINDX26)
                          & ^(CVP36527 | CVINDX27)) +
                (CVP36528) +
                (CVP36560 | CVINDX60)) +       /* RHEUM DISEASE ADDED */
/* weight=2*/
                (((CVP36519) +
...
/* weight=6*/
               ((CVP36555 | CVINDX55)*6) +
               ((CVP36561 | CVINDX61)*6);    /* AIDS ADDED */
